...
/* 로딩 스피너 태그 생성 */
function createSpinner(size = 100, loadingMsg = "로딩중") {
    return `<figure class="loadingSpinner">
					...
					<figcaption>${loadingMsg}</figcaption>
				</figure>`
}

/* 로딩 스피너를 랜더링하는 함수 */
export function renderSpinner(placeToRender, size, loadingMsg) {
    insert(placeToRender, createSpinner(size, loadingMsg), 'first');
}


/* empty user info 태그 생성 */
function createEmptySVG(size = 200, errorMessage = "데이터가 없습니다.") {
...
/* empty user info 랜더링 */
export function renderEmptySVG(placeToRender, size, errorMessage) {
    insert(placeToRender, createEmptySVG(size, errorMessage), 'last');
}