...
// ~ 보다 큼 또는 작음 → boolean
let lessThen = number[0] < number[3]; // 10 < 101 ? true
console.log(lessThen);

let greaterThen = number[0] > number[2]; // 10 > 6 ? true
console.log(greaterThen);


// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = number[3] >= number[4]; // 101 >= 10000 ? false
console.log(lessThenAndEqual);

let greaterThenAndEqual = number[3] <= number[4]; // 101 <= 10000 ? true;
console.log(greaterThenAndEqual);

console.clear();
// 동등(등급이나 정도가 같다) → boolean
let equal = number[0] == number[1]; // 10 = '10' ? true;
console.log(equal);

// 다름(같지 않음) → boolean
let unequal = number[0] != number[1]; // 10 != '10' ? false;
console.log(unequal);

// 동일(똑같다) → boolean
let same = number[0] === number[1]; // 10 === '10' ? false;
console.log(same);

// 다름(엄격) → boolean
let notSame = number[0] !== number[1]; // 10 !== '10' ? true;
console.log(notSame);
